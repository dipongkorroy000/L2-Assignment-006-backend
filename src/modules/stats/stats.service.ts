...
const getUserStats = async () => {
  ...
  const [totalUsers, totalActiveUsers, totalInActiveUsers, totalBlockedUsers, newUsersInLastWeek, newUsersInLastMonth, usersByRole] = await Promise.all([
    totalUsersPromise,
...
const getPaymentStats = async () => {
  ...
  const totalRevenuePromise = Payment.aggregate([{$match: {status: PAYMENT_STATUS.PAID}}, {$group: {_id: null, totalRevenue: {$sum: "$amount"}}}]);
...
const getSenderStats = async (email: string) => {
  const sender = await User.findOne({email});

  const totalParcels = await Parcel.countDocuments({senderId: sender?._id});

  // Count by status
  const parcelsByStatus = await Parcel.aggregate([{$match: {senderId: sender?._id}}, {$group: {_id: "$status", count: {$sum: 1}}}]);

  // Count by payment status
  const parcelsByPayment = await Parcel.aggregate([{$match: {senderId: sender?._id}}, {$group: {_id: "$payment", count: {$sum: 1}}}]);

  // Recent parcels (last 5)
  const recentParcels = await Parcel.find({senderId: sender?._id}).sort({createdAt: -1}).limit(5).select("trackingId title status payment createdAt");

  return {totalParcels, parcelsByStatus, parcelsByPayment, recentParcels};
};

export const StatsService = {getUserStats, getPaymentStats, parcelStats, getSenderStats};
